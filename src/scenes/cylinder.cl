...
#define SOURCE (double3)(0.0312859438128765, 9.027751876378526111129, 0.082347812875947)
...
Target sdf_target(double3 p, int subscene_id) {
    ...
    double scale=0.25;
    d = target_min(d, target(sdf_sphere(p-(double3)(sin(a)*scale,     -9.0, cos(a)*scale    ), 0.5), 0, 1));
    d = target_min(d, target(sdf_sphere(p-(double3)(sin(a+0.1)*scale, -9.0, cos(a+0.1)*scale), 0.5), 1, 1));
    return d;
}
...
double sdf_scene(double3 p, int subscene_id) {
    ...
    d = min(d, -sdf_cylinder(p, (double3)(0,0,0.5)));
